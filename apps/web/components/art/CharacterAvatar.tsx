 ...
 * Parse dna hex → makeRng(dna) picks fur palette, eye style, expression and
 * accessory tint. Cosmetic slugs render as layers on top. Pure & memoizable.
 */
...
const RACCOON_PALETTES: Palette[] = [
  { base: "#8E9AA8", dark: "#2A323F", light: "#D7DEE8", outline: "#161B24" },
  { base: "#7C7468", dark: "#2E2A24", light: "#D9CFC0", outline: "#17140F" },
  { base: "#6E7B8C", dark: "#232B36", light: "#C9D4E2", outline: "#121821" },
  { base: "#8A8276", dark: "#332F28", light: "#E2D9C8", outline: "#181510" },
];

const HOUND_PALETTES: Palette[] = [
  { base: "#B98A56", dark: "#6B4A2B", light: "#E4C49A", outline: "#2B1D10" },
  { base: "#A4754A", dark: "#5C3F26", light: "#D9B68C", outline: "#251810" },
  { base: "#C09A6B", dark: "#75522F", light: "#ECD3AC", outline: "#2E2113" },
];

const TINTS = ["#FFB627", "#4D9DE0", "#C792EA", "#3DDC97"];

export interface AvatarTraits {
  palette: Palette;
  eyeStyle: 0 | 1 | 2; // sly | wide | scarred
  expression: 0 | 1 | 2; // smirk | grit | deadpan
  tint: string;
}
...
function Eyes({ t, hound }: { t: AvatarTraits; hound: boolean }) {
      ...
      <g>
        {[lx, rx].map((x, i) => (
          <g key={i}>
            ...
            <path d={`M ${x - 10} ${eyeY - 4} A 10 10 0 0 1 ${x + 10} ${eyeY - 4} L ${x + 10} ${eyeY - 10} L ${x - 10} ${eyeY - 10} Z`} fill={palette.dark} />
  ...
  return (
    <g>
      ...
      <path d={`M ${lx - 10} ${eyeY - 4} A 10 10 0 0 1 ${lx + 10} ${eyeY - 4} L ${lx + 10} ${eyeY - 10} L ${lx - 10} ${eyeY - 10} Z`} fill={palette.dark} />
  ...
  if (expression === 1) {
    ...
    return (
      <g>
        ...
        <line x1="95" y1="123.5" x2="95" y2="127" stroke={palette.outline} strokeWidth="1.4" />
        <line x1="101" y1="124.5" x2="101" y2="128" stroke={palette.outline} strokeWidth="1.4" />
        <line x1="107" y1="123.5" x2="107" y2="126.5" stroke={palette.outline} strokeWidth="1.4" />
      </g>
    );
  }
  // deadpan
  ...
  return (
    <g>
      ...
      <g transform="translate(148 138) rotate(-35)">
        ...
        <rect x="-10" y="10" width="22" height="9" fill={p.dark} />
        <rect x="-10" y="28" width="22" height="9" fill={p.dark} />
        <rect x="-10" y="44" width="22" height="8" rx="4" fill={p.dark} />
      </g>
      {/* shoulders */}
      <path
        ...
        strokeWidth="3.5"
      />
      <path d="M 100 146 C 134 146 156 162 156 200 L 138 200 C 138 170 122 154 100 150 Z" fill={p.dark} opacity="0.25" />
      {/* chest fur notch */}
      <path d="M 92 152 L 100 164 L 108 152" fill="none" stroke={p.dark} strokeWidth="2.5" strokeLinecap="round" />
      {/* ears */}
      <g>
        ...
        <path d="M 58 50 C 58 42 64 38 70 40 C 68 46 66 52 65 56 Z" fill={p.dark} />
        ...
        <path d="M 142 50 C 142 42 136 38 130 40 C 132 46 134 52 135 56 Z" fill={p.dark} />
      </g>
      {/* head */}
      <path
...
function BloodhoundBust({ t }: { t: AvatarTraits }) {
  ...
  return (
    <g>
      ...
      <path d="M 100 148 C 136 148 158 164 158 200 L 140 200 C 140 172 124 156 100 152 Z" fill={p.dark} opacity="0.25" />
      {/* long droopy ears (behind head) */}
      <path d="M 52 76 C 36 84 32 120 38 148 C 46 158 60 154 64 142 C 66 118 62 92 60 80 Z" fill={p.dark} stroke={p.outline} strokeWidth="3.5" strokeLinejoin="round" />
      <path d="M 148 76 C 164 84 168 120 162 148 C 154 158 140 154 136 142 C 134 118 138 92 140 80 Z" fill={p.dark} stroke={p.outline} strokeWidth="3.5" strokeLinejoin="round" />
      {/* head — long muzzle */}
      <path
      ...
      />
      {/* forehead wrinkles */}
      <path d="M 78 62 Q 100 54 122 62" fill="none" stroke={p.dark} strokeWidth="2.2" strokeLinecap="round" opacity="0.8" />
      <path d="M 82 70 Q 100 63 118 70" fill="none" stroke={p.dark} strokeWidth="2" strokeLinecap="round" opacity="0.6" />
      {/* jowls */}
      ...
      <Eyes t={t} hound />
      <path d="M 68 102 Q 76 106 84 102" fill="none" stroke={p.dark} strokeWidth="1.8" opacity="0.65" />
      <path d="M 116 102 Q 124 106 132 102" fill="none" stroke={p.dark} strokeWidth="1.8" opacity="0.65" />
      <Mouth t={t} />
      {/* collar + badge */}
